...
def train(model,dl_train,dl_valid,epochs = 100,start=1,lr=1e-4):
    ...
    for epoch in range(start,epochs+start):  
        ...
        print("\n"+"=========="*8 + "%s"%nowtime)
        if epoch % 2 == 0:
            torch.save(model.state_dict(),'train_model_inter/model_weights_%d_%0.5f_%0.5f.pth'%(epoch,val_loss_sum/val_step,val_metric_sum/val_step))
    return dfhistory
...
output_path = r'../inter-output/output.csv'
...
# model.load_state_dict(torch.load('train_model/model_weights_16_0.04825_0.17415.pth'))

model.to(device)
his1 = train(model,dl_train,dl_valid,epochs = 50,start=1,lr=1e-5)
his1.to_csv("train_log_inter.txt")